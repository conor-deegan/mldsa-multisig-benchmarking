...
//! `OK <total_proof_bytes> <total_prove_ms> <total_verify_ms>` to stdout and exits 0;
//! on any failure it prints the error to stderr and exits 1.
//!
...
fn run() -> Result<(usize, u128, u128), String> {
    ...
    for pair in pairs.chunks(2) {
        ...
        total_prove_ms += t0.elapsed().as_millis();
        ...
        total_verify_ms += t1.elapsed().as_millis();
    }

    Ok((total_bytes, total_prove_ms, total_verify_ms))
}

fn main() -> ExitCode {
    match run() {
        Ok((bytes, prove_ms, verify_ms)) => {
            println!("OK {bytes} {prove_ms} {verify_ms}");
            ExitCode::SUCCESS
        }
        Err(e) => {
